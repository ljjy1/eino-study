...
// It returns the last assistant text content, intermediate messages for session persistence,
// the new interrupt ID (if the agent was paused again), the final msgIdx, and any error.
func StreamContinue[M adk.MessageType](w io.Writer, sessionID string, startMsgIdx int, events *adk.AsyncIterator[*adk.TypedAgentEvent[M]]) (string, []M, string, int, error) {
	surfaceID := "chat-" + sessionID
...
func streamEvents[M adk.MessageType](w io.Writer, surfaceID string, rootChildren *[]string, msgIdx *int, events *adk.AsyncIterator[*adk.TypedAgentEvent[M]]) (string, []M, string, error) {
	...
	for {
		...
		if event.Err != nil {
			if IsModelRetryInProgress(event.Err) {
		...
		if mo.IsStreaming && mo.MessageStream != nil {
			...
			for {
				...
				if recvErr != nil {
					if IsModelRetryInProgress(recvErr) {
